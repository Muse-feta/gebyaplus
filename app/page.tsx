...
export default function Home() {
  ...
  useEffect(() => {
    const fetchSession = async () => {
      ...
      if(data){
        setSession(data.session.userId);
      }
    }
    fetchSession()
  }, [])
    ...
    <div className="min-h-screen w-full relative bg-black">
        ...
        <div className="flex items-center gap-4">
          {!session ? (
            <>
              <Link
                href="/login"
                className="rounded-md font-bold relative cursor-pointer hover:-translate-y-0.5 transition duration-200 inline-block hover:text-foreground text-muted-foreground px-4 py-2 text-sm"
              >
                Log In
              </Link>
      ...
      <header className="sticky top-4 z-[9999] mx-4 flex w-auto flex-row items-center justify-between rounded-full bg-background/80 backdrop-blur-sm border border-border/50 shadow-lg md:hidden px-4 py-3">
        <a className="flex items-center justify-center gap-2" href="/">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 bg-gradient-to-br from-[#FF0050] to-[#25F4EE] rounded-lg flex items-center justify-center">
              <span className="text-white font-bold text-xs">E</span>
            </div>
            <span className="font-bold text-base text-foreground">EngageX</span>
          </div>
        </a>
